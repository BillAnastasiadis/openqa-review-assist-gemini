...
def main():
    # Show usage if not enough arguments are provided at all
    if len(sys.argv) < 3:
        usage = (
            "Usage:\n"
            "  Fetch raw code:  python trace_functions.py --raw <git_hash> <file_path>\n"
            "  Trace functions: python trace_functions.py <git_hash> <failing_file_path> <func1> [func2...]"
        )
    ...
    if sys.argv[1] == "--raw":
        if len(sys.argv) != 4:
            print(json.dumps({"error": "Usage: python trace_functions.py --raw <git_hash> <file_path>"}), flush=True)
            sys.exit(1)
            
        git_hash = sys.argv[2]
        file_path = sys.argv[3]
        results = fetch_raw_code(git_hash, file_path)
        print(json.dumps(results, indent=2), flush=True)
        sys.exit(0)
        
    # If the user wants to trace functions
    if len(sys.argv) < 4:
        print(json.dumps({"error": "Usage: python trace_functions.py <git_hash> <failing_file_path> <func1> [func2...]"}), flush=True)
        sys.exit(1)
